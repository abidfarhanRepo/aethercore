  ...
  server.get<{ Querystring: { limit?: string; offset?: string } }>(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const roleLookup = Object.fromEntries(roleCounts.map((r: any) => [r.role, r._count]))
        ...
        const total = builtInRoles.length + (await prisma.customRole.count())
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        if (id.startsWith('builtin-')) {
          ...
          const builtInRoles: Record<string, any> = {
            ADMIN: {
              id: 'builtin-admin',
              name: 'ADMIN',
              description: 'Full system access',
