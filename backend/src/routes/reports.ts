...
function setCache(key: string, data: any, ttlSeconds: number = 3600) {
...
export default async function reportsRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/api/v1/reports/sales-summary', async (req, reply) => {
    ...
    const buckets: Record<string, { sales: number; revenue: number; discount: number; tax: number }> = {}
    ...
    for (const s of sales) {
      ...
      if (!buckets[key]) buckets[key] = { sales: 0, revenue: 0, discount: 0, tax: 0 }
      buckets[key].sales += 1
  ...
  // ============ Sales by Product ============
  fastify.get('/api/v1/reports/sales-by-product', async (req, reply) => {
  ...
  // ============ Sales by Category ============
  fastify.get('/api/v1/reports/sales-by-category', async (req, reply) => {
    const { dateFrom, dateTo } = req.query as Record<string, string>
    const from = dateFrom ? new Date(dateFrom) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
    const to = dateTo ? new Date(dateTo) : new Date()
    ...
    const buckets: Record<string, { qty: number; revenue: number }> = {}
    for (const item of sales) {
      const cat = item.product.category || 'Uncategorized'
      if (!buckets[cat]) buckets[cat] = { qty: 0, revenue: 0 }
  ...
  // ============ Top Products ============
  fastify.get('/api/v1/reports/top-products', async (req, reply) => {
  ...
  // ============ Revenue Analysis ============
  fastify.get('/api/v1/reports/revenue-analysis', async (req, reply) => {
    const { dateFrom, dateTo } = req.query as Record<string, string>
    const from = dateFrom ? new Date(dateFrom) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
    const to = dateTo ? new Date(dateTo) : new Date()
  ...
  // ============ Inventory Valuation ============
  fastify.get('/api/v1/reports/inventory-valuation', async (req, reply) => {
    const cacheKey = 'inventory-valuation'
    const cached = getCache(cacheKey)
    if (cached) return cached

    const products = await prisma.product.findMany({ select: { id: true, sku: true, name: true, costCents: true, priceCents: true } })
    const out: any[] = []

    for (const p of products) {
      const qty = await prisma.inventoryLocation.aggregate({
  ...
  // ============ Inventory Movement ============
  fastify.get('/api/v1/reports/inventory-movement', async (req, reply) => {
    ...
    const result = await Promise.all(
      products.map(async (p) => {
        ...
        const current = await prisma.inventoryLocation.aggregate({
          where: { productId: p.id },
          _sum: { qty: true },
        })
  ...
  // ============ Low Stock Items ============
  fastify.get('/api/v1/reports/low-stock', async (req, reply) => {
    const cacheKey = 'low-stock'
    const cached = getCache(cacheKey)
    if (cached) return cached

    const lowStock = await prisma.inventoryLocation.findMany({
      where: { qty: { lte: prisma.inventoryLocation.fields.minThreshold } },
  ...
  // ============ Customer Analytics ============
  fastify.get('/api/v1/reports/customer-analytics', async (req, reply) => {
    const { dateFrom, dateTo } = req.query as Record<string, string>
  ...
  // ============ Payment Methods ============
  fastify.get('/api/v1/reports/payment-methods', async (req, reply) => {
    const { dateFrom, dateTo } = req.query as Record<string, string>
    const from = dateFrom ? new Date(dateFrom) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
    const to = dateTo ? new Date(dateTo) : new Date()
  ...
  // ============ Discounts Impact ============
  fastify.get('/api/v1/reports/discounts-impact', async (req, reply) => {
    const { dateFrom, dateTo } = req.query as Record<string, string>
    const from = dateFrom ? new Date(dateFrom) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
    const to = dateTo ? new Date(dateTo) : new Date()
    ...
    const discounts = await prisma.saleDiscount.groupBy({
      by: ['reason'],
      where: { sale: { createdAt: { gte: from, lte: to }, status: 'completed' } },
      _sum: { amountCents: true },
      _count: true,
  ...
  fastify.get('/api/v1/reports/employee-performance', async (req, reply) => {
    const { dateFrom, dateTo } = req.query as Record<string, string>
    const from = dateFrom ? new Date(dateFrom) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
    const to = dateTo ? new Date(dateTo) : new Date()
    ...
    const employees = await prisma.sale.groupBy({
      by: ['userId'],
      where: { createdAt: { gte: from, lte: to }, status: 'completed' },
      _sum: { totalCents: true },
      _count: true,
  ...
  // ============ Profit Margins ============
  fastify.get('/api/v1/reports/profit-margins', async (req, reply) => {
    const { dateFrom, dateTo } = req.query as Record<string, string>
    const from = dateFrom ? new Date(dateFrom) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
    const to = dateTo ? new Date(dateTo) : new Date()
    ...
    const margins = saleItems.reduce(
    ...
    const result = Object.values(margins).map((m: any) => ({
      ...m,
  ...
  // ============ Tax Summary ============
  fastify.get('/api/v1/reports/tax-summary', async (req, reply) => {
    const { dateFrom, dateTo } = req.query as Record<string, string>
    const from = dateFrom ? new Date(dateFrom) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
    const to = dateTo ? new Date(dateTo) : new Date()
    ...
    const taxData = await prisma.sale.aggregate({
      where: { createdAt: { gte: from, lte: to }, status: 'completed' },
  ...
  // ============ Hourly Sales ============
  fastify.get('/api/v1/reports/hourly-sales', async (req, reply) => {
  ...
  fastify.get('/api/v1/reports/inventory-adjustments', async (req, reply) => {
    const { dateFrom, dateTo } = req.query as Record<string, string>
    const from = dateFrom ? new Date(dateFrom) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
    const to = dateTo ? new Date(dateTo) : new Date()
  ...
  fastify.get('/api/v1/reports/daily-sales', async (req, reply) => {
    const { date } = req.query as Record<string, string>
    const reportDate = date ? new Date(date) : new Date()
    reportDate.setHours(0, 0, 0, 0)
    const nextDay = new Date(reportDate)
  ...
  // ============ CSV Export ============
  fastify.get('/api/v1/reports/export/csv', { preHandler: [requirePermission('reports.export')] }, async (req, reply) => {
    ...
    if (type === 'sales-summary') {
      ...
      for (const sale of salesData) {
        ...
        if (!grouped[date]) grouped[date] = { revenue: 0, discount: 0, tax: 0, count: 0 }
