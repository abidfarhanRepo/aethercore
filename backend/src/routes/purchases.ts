...
export default async function purchaseRoutes(fastify: FastifyInstance){
  ...
  fastify.post('/purchases', { schema: createPoSchema }, async (req, reply)=>{
  ...
  fastify.post('/purchases/:id/receive', { schema: receiveSchema }, async (req, reply)=>{
  ...
  fastify.get('/purchases/:id', { schema: { params: { type: 'object', required: ['id'], properties: { id: { type: 'string' } } } } }, async (req, reply)=>{
    const id = (req.params as any).id
