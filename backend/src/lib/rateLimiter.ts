 ...
 * More efficient than in-memory rate limiting
 */
...
export class RedisRateLimiter {
  ...
  async isAllowed(key: string): Promise<RateLimitResult> {
    ...
    try {
      ...
      const [allowed, limit, current, remainingMs] = result as [number, number, number, number];
...
export const userRateLimiter = new RedisRateLimiter({
...
export async function checkRateLimits(
  ...
  allowed: boolean;
  failedLimiter?: string;
  result?: RateLimitResult;
}> {
  // Check user limit
  if (userId) {
    const userResult = await userRateLimiter.isAllowed(userId);
  ...
  // Check IP limit
  if (ipAddress) {
    const ipResult = await ipRateLimiter.isAllowed(ipAddress);
    if (!ipResult.allowed) {
      return {
        allowed: false,
        failedLimiter: 'ip',
 ...
 */
export async function rateLimiterHook(fastify: any, options?: { strict?: boolean }) {
