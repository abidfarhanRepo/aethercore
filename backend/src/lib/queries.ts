...
/**
 * Optimized query patterns to avoid N+1 queries
 * Uses Prisma select/include wisely for efficient data fetching
 */
 ...
 */
export async function findProductsOptimized(
 ...
 */
export async function batchLoadProductInventory(productIds: string[]) {
  ...
  try {
    ...
    logger.error('Error batch loading inventory:', error);
    throw error;
  }
}
 ...
 */
export async function findSalesOptimized(options: PaginationOptions = {}) {
  const page = Math.max(1, options.page || 1);
  const limit = Math.min(options.limit || DEFAULT_PAGE_SIZE, MAX_PAGE_SIZE);
  const skip = (page - 1) * limit;
 ...
 */
export async function findUserWithRelations(userId: string) {
  const startTime = process.hrtime.bigint();
...
export async function countRecords(
  model: 'product' | 'sale' | 'user' | 'inventory',
  where?: any
) {
  const startTime = process.hrtime.bigint();
  ...
  try {
    ...
    switch (model) {
      case 'product':
        ...
        break;
      case 'sale':
        count = await prisma.sale.count({ where });
        break;
      case 'user':
        count = await prisma.user.count({ where });
        break;
      case 'inventory':
        count = await prisma.inventoryLocation.count({ where });
        break;
    }
 ...
 */
export function analyzeQueryPerformance(queryTime: number): {
...
export async function getIndexStatistics() {
  try {
    ...
    logger.error('Error fetching index statistics:', error);
    return null;
  }
}
