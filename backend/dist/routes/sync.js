...
async function createSale(data) {
    ...
    let warehouse = await db_1.prisma.warehouse.findFirst();
    if (!warehouse) {
        warehouse = await db_1.prisma.warehouse.create({
            data: {
    ...
    const sale = await db_1.prisma.sale.create({
        data: {
    ...
    for (const item of items) {
        const location = await db_1.prisma.inventoryLocation.findFirst({
            where: {
                warehouseId: warehouse.id,
                productId: item.productId,
            },
        ...
        if (location) {
            await db_1.prisma.inventoryLocation.update({
...
async function adjustInventory(data) {
    ...
    let location = await db_1.prisma.inventoryLocation.findFirst({
        where: {
    ...
    if (!location) {
        location = await db_1.prisma.inventoryLocation.create({
            data: {
                warehouseId: targetWarehouseId,
                productId,
                qty: 0,
    ...
    // Update location
    const updated = await db_1.prisma.inventoryLocation.update({
        where: { id: location.id },
        data: { qty: newQty },
    });
    // Log transaction
...
async function createProduct(data) {
    ...
    if (warehouse) {
        await db_1.prisma.inventoryLocation.create({
            data: {
 ...
 */
async function refundSale(data) {
    ...
    const refund = await db_1.prisma.return.create({
        data: {
            saleId,
            userId: sale.userId,
            items: {
 ...
 */
async function returnSale(data) {
    // Similar to refund
