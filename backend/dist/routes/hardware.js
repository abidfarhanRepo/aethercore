...
async function getSettingValue(key) {
    const row = await db_1.prisma.settings.findUnique({ where: { key } });
    return row?.value || null;
}
async function upsertJsonSetting(key, value, label, description) {
...
async function setDevices(key, devices) {
    const label = key === PRINTERS_KEY ? 'Hardware Printers' : 'Hardware Scanners';
    const description = key === PRINTERS_KEY
...
function buildTestPrintContent(printerName, settings) {
    const width = Math.max(24, settings.paperWidth);
    const separatorChar = settings.separatorStyle === 'double'
        ? '='
    ...
    if (settings.showStoreName)
        rows.push('AETHER POS SYSTEM');
    if (settings.showReceiptId)
        rows.push('Receipt #TEST-0001');
    if (settings.showTimestamp)
        rows.push(new Date().toLocaleString());
    if (settings.headerText)
        rows.push(settings.headerText);
    ...
    if (settings.showPaymentSection) {
        rows.push(line);
        rows.push('Payment');
        if (settings.showPaymentBreakdown)
            rows.push('CASH                    10.00');
        if (settings.showChange)
            rows.push('Change:                 0.01');
    }
    rows.push(line);
    if (settings.showThankYou)
        rows.push('Thank you for your purchase!');
    if (settings.showCashier)
        rows.push('Cashier: Demo Cashier');
...
async function hardwareRoutes(fastify) {
    ...
    fastify.put('/api/v1/hardware/print-settings', async (req, reply) => {
        ...
        await upsertJsonSetting(PRINT_SETTINGS_KEY, updated, 'Printer Design Settings', 'Layout and styling settings for printed receipts');
